...
def save_to_db(timetable, user_id, user_name, semester, url):
    ...
    try:
        res = requests.post(
            "http://inhuckjin85.dothome.co.kr/python/save_schedule.php",
            json=payload,
            verify=False)
        ...
        if data.get("success"):
            ...
            res2 = requests.post(
                "http://inhuckjin85.dothome.co.kr/python/save_user.php",
                json=payload2,
                verify=False)
...
def delete_to_db(user_id, semester):
    ...
    try:
        res = requests.post(
            "http://inhuckjin85.dothome.co.kr/python/delete_schedule.php",
            json=payload,
            verify=False)
        data = res.json()
        if data.get("success"):
...
def load_from_db(user_id=None, user_name=None, semester=None):
    ...
    try:
        res = requests.post(
            "http://inhuckjin85.dothome.co.kr/python/load_schedule.php",
            json=payload)
        data = res.json()
        if data.get("success"):
...
def fetch_timetable_html(url):
    ...
    driver = webdriver.Chrome(
        options=options)  # chromedriver.exe는 같은 폴더에 있어야 함
...
async def 시간표(ctx, cm2=None, *args):
    ...
    if cm2 == None:
        ...
        if category is None:
            ...
            category = await guild.create_category(name="시간표 개인방",
        ...
        if existing:
            await ctx.send(
                f"{ctx.author.mention}, 이미 개인 채널이 존재합니다: {existing.mention}")
            try:
...
```""")
    ...
    if cm2 == "추가" and args[0]:
        ...
        output += "```"
        messages.append(output)
        data = save_to_db(timetable, ctx.author.id, ctx.author.display_name,
                          semester, args[0])
        # 메시지 출력
        if data.get("success"):
    ...
    if cm2 == "확인":
        ...
        timetable = load_from_db(user_id=user_id,
                                 user_name=user_name,
                                 semester=semester)
        if not timetable:
        ...
        # 정렬 기준을 요일 순서 + 시작 시간으로 설정
        timetable.sort(key=lambda x: (day_order.get(x.get('day'), 99),
    ...
    if cm2 == "사이트접속":
        ...
        sig = hmac.new(SECRET_KEY.encode(), payload.encode(),
